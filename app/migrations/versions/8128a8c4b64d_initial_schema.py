...
def upgrade() -> None:
    # pgvector must be explicitly enabled per-database before any VECTOR
    # column type can be used — the pgvector/pgvector:pg16 image makes
    # the extension available, but doesn't turn it on automatically.
    # Must run before create_table('knowledge_base', ...) below, since
    # that table's embedding column depends on this type existing.
